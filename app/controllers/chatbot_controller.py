...
from GUI.ChatBot_GUI.models.chatbot_model import ChatBotModel
...
class ChatbotController(QObject):
    ...
    def start_message_processing(self):
        if self.main_view.input_field.text() != '' and not self.is_running:
            ...
            self.chatbot_model = ChatBotModel()      # Worker

            # Preparing the chatbot to receive the message history
            history = self.history_model.read_history()
            
            self.chatbot_model.inject_current_history(history)

            ## self.history_model.add_in_history(human_messsage_sent)    #TODO: It should be in a function called by an emission
            ...
            self.chatbot_model.update_last_message(human_messsage_sent)

            # Setting thread
            self.mssg_thread = QThread()
            self.chatbot_model.moveToThread(self.mssg_thread)

            # Signals reactions
            self.mssg_thread.started.connect(self.chatbot_model.handle_message)

            self.chatbot_model.understanding_step.connect(self.on_update_process_state)
            self.chatbot_model.finding_data_step.connect(self.on_update_process_state)
            self.chatbot_model.error_finding_data_found.connect(self.on_update_process_state)
            self.chatbot_model.interpreting_results.connect(self.on_update_process_state)
            self.chatbot_model.answer_ready.connect(self.on_message_ready)

            # Finishg threads
            self.chatbot_model.message_processing_finished.connect(self.mssg_thread.quit)
            self.chatbot_model.message_processing_finished.connect(self.chatbot_model.deleteLater)
    ...
    def on_message_ready(self, final_mssg):
        ...
        last_interaction = self.chatbot_model.get_last_interaction()
